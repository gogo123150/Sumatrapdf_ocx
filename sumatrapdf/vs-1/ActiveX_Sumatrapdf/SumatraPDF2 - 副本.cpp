...
static void RememberDefaultWindowPosition(WindowInfo& win)
{
    ...
    if (WIN_STATE_NORMAL == gGlobalPrefs->windowState &&
		...
		RECT rc;
		::GetClientRect(win.hwndFrame,&rc);

		gGlobalPrefs->windowPos.x =rc.left;gGlobalPrefs->windowPos.y=rc.top;
		gGlobalPrefs->windowPos.dx=rc.right - rc.left;gGlobalPrefs->windowPos.dy=rc.bottom - rc.top;
    }
}
...
static WindowInfo* CreateWindowInfo()
{
	...
	gGlobalPrefs->windowPos.x =x;gGlobalPrefs->windowPos.y=y;
	gGlobalPrefs->windowPos.dx=dx;gGlobalPrefs->windowPos.dy=dy;
    ...
    HWND hwndFrame = CreateWindow(
			...
			x,y,dx,dy,/*windowPos.x, windowPos.y, windowPos.dx, windowPos.dy,*/
            ghWnd/*NULL*/, NULL,
            ghinst, NULL);
    if (!hwndFrame)
        return NULL;
...
static void GetCommandLineInfo(CommandLineInfo& i)
{
	...
	ScopedMem<WCHAR> cmdLine(str::Format(L"\"%s\" -plugin \"%s\" %d \"%s\"",
		L"E:\\project\\sumatrapdf\\obj-dbg\\SumatraPDF.exe", L"", ghWnd, L""));
...
int  Entry(HINSTANCE hInstance,/* HINSTANCE hPrevInstance,*/ LPSTR lpCmdLine, int nCmdShow)
{
    ...
    for (size_t n = 0; n < i.fileNames.Count(); n++) {
        if (i.reuseInstance && !i.printDialog) {
            ...
            pwin = LoadOnStartup(i.fileNames.At(n), i, isFirstWin);
            if (!pwin) {
                retCode++;
                continue;
            }
            if (i.printDialog)
                OnMenuPrint(pwin, i.exitWhenDone);
        }
        isFirstWin = false;
    }
    ...
    if (isFirstWin)
        UpdateToolbarAndScrollbarState(*pwin);

    // Make sure that we're still registered as default,
    // if the user has explicitly told us to be
    if (gGlobalPrefs->associatedExtensions && pwin)
        RegisterForPdfExtentions(pwin->hwndFrame);
    ...
    if (i.stressTestPath) {
        ...
        RebuildMenuBarForWindow(pwin);
        StartStressTest(&i, pwin, &gRenderCache);
    }
...
void TestChildWindow(HINSTANCE hInstance)
{
	 WNDCLASSEX  wcex;
    ATOM        atom;
...
void OnMoveFrame()
{
	SendMessage(pwin->hwndFrame, WM_SIZE, 0, 0);
}