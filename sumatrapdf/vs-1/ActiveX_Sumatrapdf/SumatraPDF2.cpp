...
static bool LoadDocIntoWindow(LoadArgs& args, PasswordUI *pwdUI, DisplayState *state = NULL)
{
	...
	if (engine)
		win->dm = new DisplayModel2(engine, engineType, win);//new DisplayModel(engine, engineType, win);
	else
		win->dm = NULL;
	...
	*/
	...
	if (pageCount > 0) {
		UpdateToolbarPageText2(win, pageCount);
		UpdateToolbarFindText2(win);
	}
...
static void UpdateToolbarAndScrollbarState(WindowInfo& win)
{
	ToolbarUpdateStateForWindow2(&win, true);
...
static WindowInfo* CreateWindowInfo()
{
	...
	WindowInfo *win = new WindowInfo2(hwndFrame);//new WindowInfo(hwndFrame);
...
static WindowInfo* LoadDocumentOld(LoadArgs& args)
{
	...
	bool loaded = false;
	if (args.fileName == NULL || wcscmp(args.fileName, L""))
	{
	...
	win->prevDisplayMode = DM_SINGLE_PAGE;
	return win;
}
...
void WindowInfo::PageNoChanged(int pageNo)
{
	...
	if (INVALID_PAGE_NO != pageNo) {
		...
		ToolbarUpdateStateForWindow2(this, false);
		if (dm->engine && dm->engine->HasPageLabels())
			UpdateToolbarPageText2(this, dm->PageCount(), true);
	}
...
int _sign = 0; //1 单页签章,2所有页签章,0不签章
static int map_x = 0;
static int map_y = 0;
static void  DrawSign(WindowInfo& win, HDC hdc, LPCWSTR image)
{

	BITMAP bmp;
	HBITMAP mybitmap = (HBITMAP)LoadImage(NULL, image, IMAGE_BITMAP,
		0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE);
	HDC ImageDC = CreateCompatibleDC(hdc);
	HBITMAP oldmap = (HBITMAP)SelectObject(ImageDC, mybitmap);
	GetObject(mybitmap, sizeof(BITMAP), &bmp);
	TransparentBlt(hdc, map_x, map_y, bmp.bmWidth, bmp.bmHeight, ImageDC, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(0, 0, 0));  //注意图片大小
	SelectObject(ImageDC, oldmap);
	DeleteDC(ImageDC);
...
int drawSignImage(WindowInfo& win, HDC hdcCanvas, HDC hdc)
{
	...
	float fZoomFactor = win.dm->ZoomReal(win.currPageNo);
	HDC bmpDC = CreateCompatibleDC(win.buffer->GetDC());
	BITMAP bmSign;
	HBITMAP hbtSignImage = win.dm->hbtSignImage;
	::GetObject(hbtSignImage, sizeof(bmSign), &bmSign);
	SelectObject(bmpDC, hbtSignImage);
	RectD rcDOfPic(0, 0, bmSign.bmWidth, bmSign.bmHeight);
	RectI rcIOfPic = win.dm->CvtToScreen(win.currPageNo, rcDOfPic);
	PointI ptI(win.dm->nMousePosX - rcIOfPic.dx / 2, win.dm->nMousePosY - rcIOfPic.dy / 2);
	PointD ptD = win.dm->CvtFromScreen(ptI, win.currPageNo);
	RectD rcD(ptD.x, ptD.y, bmSign.bmWidth, bmSign.bmHeight);
	RectI rcI = win.dm->CvtToScreen(win.currPageNo, rcD);
	TransparentBlt(hdc, rcI.x, rcI.y, rcI.dx, rcI.dy, bmpDC, 0, 0, bmSign.bmWidth, bmSign.bmHeight, 0xFFFFFF);
	DeleteDC(bmpDC);
	//   win.RepaintAsync();
	return 1;
}
static void DrawDocument(WindowInfo& win, HDC hdc, RECT *rcArea)
{
	...
	if (win2->showSelection2)
		PaintSelection2(&win, hdc);
...
static void OnMouseMove(WindowInfo& win, int x, int y, WPARAM flags)
{
	...
	{
		RECT rect;
		GetClientRect(win.hwndCanvas, &rect);
		POINT pRightTop;
		POINT pLeftBottom;
		pRightTop.x = rect.right;
		pRightTop.y = rect.top;
		pLeftBottom.x = rect.left;
		pLeftBottom.y = rect.bottom;
		ClientToScreen(win.hwndCanvas, &pRightTop);
		ClientToScreen(win.hwndCanvas, &pLeftBottom);
		rect.left = pLeftBottom.x;
		rect.right = pRightTop.x;
		rect.top = pRightTop.y;
		rect.bottom = pLeftBottom.y;
		ClipCursor(&rect);
		SetCursor(gCursorDrag);
		win.dm->nMousePosX = x;
		win.dm->nMousePosY = y;;
...
static LPCWSTR decode(const char* encodedStr, unsigned int codePage)
{
	wchar_t* wszClassName;
	wszClassName = (wchar_t*)malloc(strlen(encodedStr) + 1);
	memset(wszClassName, 0, sizeof(wszClassName));
	MultiByteToWideChar(CP_ACP, 0, encodedStr, strlen(encodedStr) + 1, wszClassName,
		strlen(encodedStr) + 1);
	//MultiByteToWideChar(CP_ACP, 0, encodedStr, strlen(encodedStr) + 1, wszClassName,
...
static void OnMouseLeftButtonUp(WindowInfo& win, int x, int y, WPARAM key)
{
	...
	PointD pt = win.dm->CvtFromScreen(PointI(x, y), win.currPageNo);
...
void BeginSign(WindowInfo& win)
{
	win.sign = 1;
	win.dm->bSign = true;
	win.dm->hbtSignImage = (HBITMAP)LoadImage(NULL, L"C:\\Users\\Administrator\\Pictures\\2.bmp", IMAGE_BITMAP,
		0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE);
	if (win.dm->hbtSignImage)
	{
		//win->dm->bSign = true;
		//saveExternalPdfFile(win->hwndFrame, szPath, false);
	   // saveOldInfo(win);
		// 锁定鼠标
		GetClipCursor(&m_oldRect);
		RECT rect;
		GetClientRect(win.hwndCanvas, &rect);
		POINT pRightTop;
		POINT pLeftBottom;
	...
	//PAINTSTRUCT ps;
	//BeginPaint(win.hwndCanvas, &ps);
	//PreRender(win, win.buffer->GetDC(), &ps.rcPaint);
	...
	//}
	//BITMAP bmp;
	//HBITMAP mybitmap = (HBITMAP)LoadImage(NULL, L"C:\\Users\\Administrator\\Pictures\\2.bmp", IMAGE_BITMAP,
	//	0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE);
	//HDC ImageDC = CreateCompatibleDC(win.buffer->GetDC());
	//HBITMAP oldmap = (HBITMAP)SelectObject(ImageDC, mybitmap);
	//GetObject(mybitmap, sizeof(BITMAP), &bmp);

	//TransparentBlt(win.buffer->GetDC(), map_x, map_y, bmp.bmWidth, bmp.bmHeight, ImageDC, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(0, 0, 0));  //注意图片大小
	//SelectObject(ImageDC, oldmap);
	//DeleteDC(ImageDC);
	//EndPaint(win.hwndCanvas, &ps);
	//MessageBox(NULL, L"Sign Page", NULL, 0);
}
static void OnPaint(WindowInfo& win)
{
	...
	if (win.IsAboutWindow()) {
		...
		default:
			...
			if (win.sign > 0)
			{
				drawSignImage(win, hdc, win.buffer->GetDC());
			}
			win.buffer->Flush(hdc);
		}
	}
...
static void UpdateUITextForLanguage()
{
	for (size_t i = 0; i < gWindows.Count(); i++) {
		WindowInfo *win = gWindows.At(i);
		UpdateToolbarPageText2(win, -1);
		UpdateToolbarFindText2(win);
		UpdateToolbarButtonsToolTipsForWindow2(win);
...
static LRESULT CALLBACK WndProcCanvas(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	static int x_x = 0;
	static int y_y = 0;
	static bool mouse_in_map = false;
	static bool mouse_l_down = false;
	WindowInfo *win = FindWindowInfoByHwnd(hwnd);
	if (!win)
...
#endif
//执行外部程序，返回结果
static void  ExecuteOtherApp(LPCWSTR appName, LPWSTR arguments, LPSTR data, int& dataLen)
{
	HANDLE hRead, hWrite;
	SECURITY_ATTRIBUTES sa;
	sa.nLength = sizeof(SECURITY_ATTRIBUTES);
	...
	if (!CreateProcess(appName, arguments, NULL, NULL, TRUE, NULL, NULL, NULL, &si, &pi)) //创建子进程
	{
		MessageBox(NULL, L"CreateProcess Failed!", L"提示", MB_OK | MB_ICONWARNING);
		return;
	}
	...
	DWORD bytesRead; //返回的长度
	int len = 0;
	while (true)
	{
		if (ReadFile(hRead, buffer, 8196, &bytesRead, NULL) == NULL) //读取管道
			break;
		strcat_s(data, EXELENGTH, buffer);
		len += bytesRead;
		Sleep(100);
	}
	dataLen = len;
...
{
	wchar_t PF[255] = { 0 };
	wchar_t signExe[400] = { 0 };
	//GetSystemDirectory(system32, 255);
	SHGetSpecialFolderPath(0,PF,CSIDL_PROGRAM_FILES, FALSE);
	wcscpy_s(signExe, 400, PF);
	wcscat_s(signExe, 400, L"\\VerySign\\SignPdf.exe");
	LPSTR retData = (LPSTR)malloc(EXELENGTH);
	ZeroMemory(retData, EXELENGTH);
	int len;
	//ExecuteOtherApp(signExe, L" -cert", retData, len);

	win->certInfos.clear();
	ifstream in(L"d:\\a.txt", ios::binary);
	Json::Reader reader;
	Json::Value root;
	int size = 0;
	if (reader.parse(in, root))  // reader将Json字符串解析到root，root将包含Json里所有子元素  
	{
		...
		for (Json::Value::Members::iterator iterMember = members.begin(); iterMember != members.end(); iterMember++)   // 遍历每个key
		{
			...
			{
				std::string strVal = root[strKey.c_str()].asString();
				CertInfo info;
				info.certName = decode(strKey.c_str(), CP_UTF8);
				info.certSerial = decode(strVal.c_str(), CP_UTF8);
	...
	//if (size>0)
	//{
	//	win->certSelect.certName = win->certInfos[0].certName;
	//	win->certSelect.certSerial = win->certInfos[0].certSerial;
	//}
...
static void RunMessageLoop(void *dummy)
{
	ScopedGdiPlus gdiPlus(false);
...
static void GetCommandLineInfo(CommandLineInfo& i)
{
	...
	ScopedMem<WCHAR> cmdLine(str::Format(L"\"%s\" -plugin \"%s\" %d \"%s\"",
		L"E:\\project\\sumatrapdf\\obj-dbg\\SumatraPDF.exe", L"", (HWND)ghWnd, L""));// L"C:\\Users\\Administrator\\Desktop\\期刊2.pdf"));
#else
	ScopedMem<WCHAR> cmdLine(str::Format(L"\"%s\" -plugin \"%s\" %d ",
		L"E:\\project\\sumatrapdf\\obj-dbg\\SumatraPDF.exe", L"", (HWND)ghWnd));// L"C:\\Users\\Administrator\\Desktop\\期刊2.pdf"));
#endif
	i.ParseCommandLine(cmdLine);
}
...
int Entry(HINSTANCE hInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (i.fileNames.Count() == 0)
	{
		win = LoadOnStartup(NULL, i, isFirstWin);
		isFirstWin = false;
	}
	else
	{
	...
	_beginthread(RunMessageLoop, 0, NULL);
	return 0;
...
void Exit()
{
	...
	{
		SetParent(gWindows.At(i)->hwndFrame,NULL);
	}*/
...
void OnFrameSize(int cx, int cy)
{
	for (ULONG i = 0; i < gWindows.Count();i++)
	{
		//SendMessage(gWindows.At(i)->hwndFrame, WM_SIZE, 0, MAKELONG(cx, cy));
		MoveWindow(gWindows.At(i)->hwndFrame, 0, 0, cx, cy, FALSE);
	}


}

LONG OnOpenFile(LPCTSTR filename)
{
	//WindowInfo *win = FindWindowInfoByHwnd(hwnd);
	...
	{
		SetFocus(win->hwndFrame);
		const WCHAR *url = filename;//= GetStaticLink(win.staticLinks, x, y);
		/*if (url && url == win->url) {
			if (str::Eq(url, SLINK_OPEN_FILE))
				SendMessage(win->hwndFrame, WM_COMMAND, IDM_OPEN, 0);
			else if (str::Eq(url, SLINK_LIST_HIDE)) {
...
{
	return OnSearchWord(gWindows.At(0), word, color, Whole, Cur, page);
}

void ClearSearchWord(LPCTSTR word)
{
	OnClearSearchWord(gWindows.At(0), word);
}