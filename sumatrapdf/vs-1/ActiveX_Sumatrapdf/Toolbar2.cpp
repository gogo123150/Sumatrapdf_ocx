...
static ToolbarButtonInfo gToolbarButtons2[] = {
...
//    { 1,   IDM_PRINT,             _TRN("Print"),          MF_REQ_PRINTER_ACCESS },
    ...
    { 0,   IDM_GOTO_PREV_PAGE,    _TRN("Previous Page"),  0 },
    { 1,   IDM_GOTO_NEXT_PAGE,    _TRN("Next Page"),      0 },
    { -1,  0,                     NULL,                   0 },
    { 2,   IDT_VIEW_FIT_WIDTH,    _TRN("Fit Width and Show Pages Continuously"), 0 },
    { 3,   IDT_VIEW_FIT_PAGE,     _TRN("Fit a Single Page"), 0 },
    { 4,   IDT_VIEW_ZOOMOUT,      _TRN("Zoom Out"),       0 },
    { 5,   IDT_VIEW_ZOOMIN,       _TRN("Zoom In"),        0 },
    { -1,  IDM_FIND_FIRST,        NULL,                   0 },
    { 6,   IDM_FIND_PREV,         _TRN("Find Previous"),  0 },
    { 7,   IDM_FIND_NEXT,         _TRN("Find Next"),      0 },
    { 8,   IDM_FIND_MATCH,        _TRN("Match Case"),     0 },
    { 9,   IDC_SIGN_PAGE,         _TRN("SIGN PAGE"),      0 },
	/*{ 9,   IDM_OPEN,              _TRN("Open"),           MF_REQ_DISK_ACCESS },*/
};

#define TOOLBAR_BUTTONS_COUNT_2 dimof(gToolbarButtons2)
...
{
    switch (gToolbarButtons2[buttonNo].cmdId) {
...
{
    int cmdId = gToolbarButtons2[buttonNo].cmdId;

    // If restricted, disable
    if (!HasPermission(gToolbarButtons2[buttonNo].flags >> PERM_FLAG_OFFSET))
        return false;
...
static TBBUTTON TbButtonFromButtonInfo(int i) {
    TBBUTTON tbButton = { 0 };
    tbButton.idCommand = gToolbarButtons2[i].cmdId;
    if (TbIsSeparator(gToolbarButtons2[i])) {
        tbButton.fsStyle = TBSTYLE_SEP;
    } else {
        tbButton.iBitmap = gToolbarButtons2[i].bmpIndex;
        tbButton.fsState = TBSTATE_ENABLED;
        tbButton.fsStyle = TBSTYLE_BUTTON;
        tbButton.iString = (INT_PTR)trans::GetTranslation(gToolbarButtons2[i].toolTip);
    }
    return tbButton;
}
...
void UpdateToolbarButtonsToolTipsForWindow2(WindowInfo *win)
{
    ...
    LRESULT res;
    for (int i = 0; i < TOOLBAR_BUTTONS_COUNT_2; i++) {
        WPARAM buttonId = (WPARAM)i;
        const char *txt = gToolbarButtons2[i].toolTip;
        if (NULL == txt)
            continue;
...
void ToolbarUpdateStateForWindow2(WindowInfo *win, bool showHide)
{
    ...
    for (int i = 0; i < TOOLBAR_BUTTONS_COUNT_2; i++) {
        if (showHide) {
            ...
            SendMessage(win->hwndToolbar, TB_HIDEBUTTON, gToolbarButtons2[i].cmdId, hide);
        }
        if (TbIsSeparator(gToolbarButtons2[i]))
            continue;

        LPARAM buttonState = IsToolbarButtonEnabled(win, i) ? enabled : disabled;
        SendMessage(win->hwndToolbar, TB_ENABLEBUTTON, gToolbarButtons2[i].cmdId, buttonState);
    }
...
static LRESULT CALLBACK WndProcFindBox(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (WM_CHAR  == message ||
        ...
        WM_UNDO  == message ||
        WM_KEYUP == message) {
        ToolbarUpdateStateForWindow2(win, false);
    }
...
void UpdateToolbarPageText2(WindowInfo *win, int pageCount, bool updateOnly)
{
...
#else
	RECT r1;
	SendMessage(win->hwndToolbar, TB_GETRECT, IDM_GOTO_PREV_PAGE, (LPARAM)&r1);
	SendMessage(win->hwndToolbar, TB_GETRECT, IDM_GOTO_PAGE, (LPARAM)&r);
	
	r.left = r1.left - r1.right;//21 +16+32;
	r.right = r.left + r1.right - r1.left;//21 +16+32;
	r.bottom = r1.bottom;
	r.top = r1.top;
#endif
    int pos_x = r.right + 10;
...
void CreateToolbar2(WindowInfo *win)
{
    HWND hwndToolbar = CreateWindowEx(0, TOOLBARCLASSNAME, NULL, WS_TOOLBAR,
                                      0, 0, 0, 0, win->hwndFrame,(HMENU)IDC_TOOLBAR2, ghinst, NULL);
    win->hwndToolbar = hwndToolbar;
    SendMessage(hwndToolbar, TB_BUTTONSTRUCTSIZE, (WPARAM)sizeof(TBBUTTON), 0);

    ShowWindow(hwndToolbar, SW_SHOW);
    TBBUTTON tbButtons[TOOLBAR_BUTTONS_COUNT_2];
    ...
    HBITMAP hbmp = LoadExternalBitmap(ghinst, L"toolbar_12.bmp", IDB_TOOLBAR2);
    ...
    //    gToolbarButtons2[0].bmpIndex = 12;
    //    gToolbarButtons2[0].cmdId = IDM_SAVEAS;
    //    gToolbarButtons2[0].toolTip = _TRN("Save As");
    //    gToolbarButtons2[0].flags = MF_REQ_DISK_ACCESS;
    //}
    for (int i = 0; i < TOOLBAR_BUTTONS_COUNT_2; i++) {
        tbButtons[i] = TbButtonFromButtonInfo(i);
        if (gToolbarButtons2[i].cmdId == IDM_FIND_MATCH)
    ...
    SendMessage(hwndToolbar, TB_ADDBUTTONS, TOOLBAR_BUTTONS_COUNT_2, (LPARAM)tbButtons);
    ...
    rbBand.cxMinChild = (rc.right - rc.left) * TOOLBAR_BUTTONS_COUNT_2;
